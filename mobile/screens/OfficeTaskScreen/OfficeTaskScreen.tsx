...
interface IOfficeTaskScreenProps {}

interface IOfficeTask {
  title: string;
  description?: string;
  createdAt: number;
  priority?: string;
...
const OfficeTaskScreen: React.FC<IOfficeTaskScreenProps> = () => {
  const { isOpen, onOpen, onClose } = useDisclose();
  const [tasks, setTasks] = useState<IOfficeTask[]>([]);
  ...
  const handleTaskDone = (idx: number) => {
    let newTasks = tasks.map((item: IOfficeTask, i: number) => {
  ...
  return (
    ...
    >
      ...
      <View>
        ...
        {tasks.map((item: IOfficeTask, idx: number) => (
          <View
            key={idx}
            style={{
              display: "flex",
...
export default OfficeTaskScreen;
